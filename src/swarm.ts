...
const FADE = 0.5;
const RIPPLE = 1.5;                // 死亡涟漪：每单位距离的延迟秒数（离命中点近的先死，跨整簇形成梯度）
...
const VEL_SHADER = /* glsl */ `
  ...
  void main() {
    ...
    vec3 v = vel.xyz;
    if (death > 0.0) {
      // 死亡：所有虫【立即】向命中点外炸散（没有一只僵住），近命中点的推力更大 → 先被撕开
...
const RENDER_VERT = /* glsl */ `
  ...
  void main() {
    ...
    if (state <= 0.0) {
      if (death > 0.0) {
        // 命中点涟漪淡出：近的先熄灭，往外一圈圈死；延迟短且封顶，不满显示滞留
        vec3 impact = skelRow(slot, 0.875).xyz; // ROW3
        float delay = min(length(pos - impact) * ${RIPPLE.toFixed(1)}, 0.4) + rank01 * 0.06;
...
export class SwarmSea {
  ...
  render(dt: number, time: number) {
    ...
    for (let s = 0; s < SLOTS; s++) {
      ...
      const mt = SLOTS * 4 + t;
      if (!slot.owner) {
        // 回收等到整簇涟漪淡完（最远延迟 0.46 + FADE），否则尾部虫会半路突然消失
        if (slot.deathAt > 0 && time > slot.deathAt + FADE + 0.6) {
          slot.deathAt = 0;
          this.skelData[mt] = 0;
